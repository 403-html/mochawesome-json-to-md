...
- Node.js >= 20 and npm available in your environment.
...
| `--fail-on-failures` | `-f`      | Exit with a non-zero code if the report contains failed tests | No           | false                  |
...
| `{{passRate}}`               | Percentage of passed tests out of total tests (e.g. `83.33`, no `%` suffix) |
